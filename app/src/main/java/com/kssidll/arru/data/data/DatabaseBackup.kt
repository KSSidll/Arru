    ...
    companion object {
        ...
        fun fromFile(file: File): DatabaseBackup {
            if (file.extension != "db")
                error(
                    "tried to create a database file representation from a file that isn't a database"
                )
            ...
            return DatabaseBackup(
                ...
                time =
                    timeFromName(name)
                        ?: error(
                            "couldn't parse time from name, naming schema must have changed without updating parsing logic"
                        ),
                totalTransactions =
                    transactionsFromName(name)
                        ?: error(
                            "couldn't parse transactions from name, naming schema must have changed without updating parsing logic"
                        ),
                totalSpending =
                    spendingFromName(name)
                        ?: error(
                            "couldn't parse spending from name, naming schema must have changed without updating parsing logic"
                        ),
        ...
        private fun spendingFromName(name: String): Long? {
            return name.split(FILE_NAME_SEPARATOR).getOrNull(4)?.toLongOrNull()
        }

        private fun timeFromName(name: String): Long? {
            return name.split(FILE_NAME_SEPARATOR).getOrNull(5)?.toLongOrNull()
        }

        private fun lockedFromName(name: String): Boolean? {
            return name.split(FILE_NAME_SEPARATOR).getOrNull(6)?.toBooleanStrictOrNull()
        }
    }
}
