...
@Database(
    ...
    autoMigrations =
        [
            ...
            AutoMigration(from = 4, to = 5, spec = MIGRATION_4_5_SPEC::class),
        ],
)
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
         ...
         * @param name name of the database or absolute path if not internal location, defaults to
         *   internal location with [DATABASE_NAME] name
         * @return [RoomDatabase.Builder] of [AppDatabase] for [name]
         */
        fun builder(context: Context, name: String = DATABASE_NAME): Builder<AppDatabase> {
        ...
        /**
         * @param context app context
         * @return [AppDatabase] created in internal location with [DATABASE_NAME] name, doesn't
         *   ensure database file creation
         */
        ...
        fun copy(fromDbFile: File, toDbFile: File) {
            ...
            fromDbFile.copyTo(toDbFile, true)

            if (fromDbWalFile.exists()) {
                fromDbWalFile.copyTo(toDbWalFile, true)
            }

            if (fromDbShmFile.exists()) {
                fromDbShmFile.copyTo(toDbShmFile, true)
            }
        }

        /**
         * moves database files and backups from [fromDbFile] parent directory to [toDbFile] parent
         * directory
         *
         * @param context app context
         * @param fromDbFile absolute path to db file from whose parent directory to move database
         ...
         */
        fun move(context: Context, fromDbFile: File, toDbFile: File) {
            var availableBackups: List<DatabaseBackup> = emptyList()
            val fromDbBackup =
                File(fromDbFile.parentFile!!.absolutePath.plus("/$DATABASE_BACKUP_DIRECTORY_NAME"))
            val toDbBackup =
                File(toDbFile.parentFile!!.absolutePath.plus("/$DATABASE_BACKUP_DIRECTORY_NAME"))
            ...
            copy(fromDbFile, toDbFile)

            availableBackups.forEach {
                copy(it.file, File(toDbBackup.absolutePath.plus("/${it.name}.db")))
            }
            ...
            if (!toDbFile.exists())
                throw IllegalStateException(
                    "Failed to copy the database from [${fromDbFile.absolutePath}] to [${toDbFile.absolutePath}]"
                )
            ...
            availableBackups.forEach { delete(it.file) }
        }
        ...
        fun lockDbBackup(databaseBackup: DatabaseBackup) {
            ...
            databaseBackup.file.renameTo(File(databaseBackup.file.parent, newName))

            if (walFile.exists()) {
                walFile.renameTo(File(databaseBackup.file.parent, newName.plus("-wal")))
            }

            if (shmFile.exists()) {
                shmFile.renameTo(File(databaseBackup.file.parent, newName.plus("-shm")))
            }
        }
        ...
        suspend fun saveDbBackup(
            ...
            copy(currentDbFile, backupDbFile)
                ...
                if (files != null) {
                    ...
                    dbFiles.forEach {
                        if (!it.hasLockMarkInName) {
                            unlockDbBackup(
                                it
                            ) // unlock backup files that don't have lock status in the name
                            changed = true
                        }
                    }
            ...
            val externalDbFile =
                File(
                    context
                        ...
                        .absolutePath
                        .plus("/database/arru_database.db")
                )
            ...
            runBlocking {
                ...
                backups.forEach { backup ->
                    if (backup.hasLockMarkInName) {
                        val walFile =
                            File(backup.file.absolutePath.replace("true", "false").plus("-wal"))
                        val shmFile =
                            File(backup.file.absolutePath.replace("true", "false").plus("-shm"))

                        if (walFile.exists()) {
                            walFile.renameTo(File(backup.file.absolutePath.plus("-wal")))
                        }

                        if (shmFile.exists()) {
                            shmFile.renameTo(File(backup.file.absolutePath.plus("-shm")))
                        }
                    }
                }
            }
    ...
    override fun migrate(db: SupportSQLiteDatabase) {
        ...
        try {
            runBlocking {
                ...
                backups.forEach { backup -> AppDatabase.lockDbBackup(backup) }
            }
        } catch (_: Exception) {}
