...
fun ProductPriceCompareChart(
    ...
    val legendItemLabelComponent =
        rememberTextComponent(vicoTheme.textColor, textSize = 12.sp, typeface = Typeface.MONOSPACE)
    ...
    LaunchedEffect(items.size) {
        ...
        filteredData.forEach { line ->
            if (line.value.size == 1) {
                ...
                line.value.add(FloatFloatPair(value.first + 1, value.second))
            }
        }
        ...
        if (filteredData.isNotEmpty()) {
            chartEntryModelProducer.runTransaction {
                ...
                extras { extraStore -> extraStore[LegendLabelKey] = filteredData.map { it.key } }
            }
        }
    }

    AnimatedVisibility(visible = linesOnChart >= 1, enter = fadeIn(), exit = fadeOut()) {
        CartesianChartHost(
            ...
            zoomState = rememberVicoZoomState(zoomEnabled = false, initialZoom = Zoom.fixed(0.03f)),
            chart =
