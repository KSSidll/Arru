...
fun Long.formatToCurrency(
    ...
    return NumberFormat.getCurrencyInstance(locale).format(this).dropLast(dropAmount)
}
...
fun Float.formatToCurrency(
    ...
    val dropAmount = if (dropDecimal) Currency.getInstance(Locale.getDefault()).defaultFractionDigits + 1 else 0
    return NumberFormat.getCurrencyInstance(locale).format(this).dropLast(dropAmount)
}
